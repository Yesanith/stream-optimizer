...
from ui.widgets import Card, Pill, Tile, fit_wraplength, text_label
...
OBS_HINT = ("In OBS: Settings > Output (Output Mode: Advanced) > Streaming for the encoder, Settings > Video for resolution and FPS. "
            "Or export a profile and load it with Profile > Import (OBS 31 or newer).")
...
class SettingsCard(Card):
    def __init__(self, master: Misc, on_copy: Callable[[], None], on_export: Callable[[], None]) -> None:
        super().__init__(master, "Recommended OBS Settings")
        self._pill = Pill(self.header, "Waiting")
        self._pill.grid(row=0, column=1, padx=(0, 8))
        self._export = ctk.CTkButton(self.header, text="Export OBS Profile", width=150, height=28, corner_radius=8, font=font(13, "bold"),
                                     fg_color=COLORS["accent"], hover_color=COLORS["accent_hover"], command=on_export, state="disabled")
        self._export.grid(row=0, column=2, padx=(0, 8))
        ...
        self._copy.grid(row=0, column=3)
        ...
        hint = text_label(body, OBS_HINT, 12, color="muted")
        hint.grid(row=2, column=0, columnspan=4, sticky="ew", padx=4, pady=(6, 0))
        fit_wraplength(body, [hint], padding=8)
    ...
    def show(self, rec: Recommendation) -> None:
        ...
        self._export.configure(state="normal")
